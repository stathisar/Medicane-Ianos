...
df = pd.read_csv('/media/stathis/StathisUSB/final_classification_march_9/training_data/new_version_merged_complete_march_21_all_sources_no_duplicates.csv')
...
custom_checkpoint = CustomCheckpoint(
    filepath='/media/stathis/StathisUSB/final_classification_march_9/models/15/lstm_identification_best_model.h5',
...
# Callbacks
csv_logger = CSVLogger('/media/stathis/StathisUSB/final_classification_march_9/models/15/identification_model_log.csv', append=False)
early_stopping = EarlyStopping(monitor='val_accuracy', patience=30, mode='max', verbose=1, restore_best_weights=True)



# Callbacks
#csv_logger = CSVLogger('/media/stathis/StathisUSB/final_classification_march_9/models/10/consequences_model_log.csv', append=False)
#checkpoint = ModelCheckpoint('/media/stathis/StathisUSB/final_classification_march_9/models/10/lstm_conseq_best_model.h5', monitor='val_accuracy', save_best_only=True, mode='max', verbose=1)
#early_stopping = EarlyStopping(monitor='val_accuracy', patience=30, mode='max', verbose=1, restore_best_weights=True)
history = model.fit(
    X_train, Y_train,
    epochs=100,
...
best_model = load_model('/media/stathis/StathisUSB/final_classification_march_9/models/15/lstm_identification_best_model.h5')
...
df.to_csv('/media/stathis/StathisUSB/final_classification_march_9/results/15/identification_lstm.csv')
